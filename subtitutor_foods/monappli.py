"""
Ce module gère la navigation de l'application entre différents menus
"""
...
class MonApplication:
    """ Cette classe représente la classe de l'objet de l'application """

    def __init__(self, cursor, cnx):

        # Releatif à la connexion à la base de donnée
        self.curseur = cursor
        self.connexion = cnx

        # les gestionnaires
        self.gest_categories = gestionnaires.Gestionnaires()
        self.gest_produits = gestionnaires.Gestionnaires()
        self.gest_magasins = gestionnaires.Gestionnaires()
        self.gest_favoris = gestionnaires.Gestionnaires()
            ...
            # L'application se trouve au menu des catégories
                ...
                if entree_util == "s":
                    ...
                    if (self.lignes_cat_bd % self.taille_page_catego) == 0:
                        if self.page_catego < (
                            self.lignes_cat_bd // self.taille_page_catego
                        ):
                            self.page_catego += 1
                        statut = self.menu1()
                    elif (self.lignes_cat_bd % self.taille_page_catego) > 0:
                        if self.page_catego < (
                            (self.lignes_cat_bd // self.taille_page_catego) + 1
                        ):
                            self.page_catego += 1
                        statut = self.menu1()
            ...
            # L'application se trouve au menu des produits
                ...
                if entree_util == "s":
                    if (self.lignes_prod_cat_bd % self.taille_page_produ) == 0:
                        if self.page_produ < (
                            self.lignes_prod_cat_bd // self.taille_page_produ
                        ):
                            self.page_produ += 1
                        ...
                        if self.page_produ < (
                            (self.lignes_prod_cat_bd // self.taille_page_produ) + 1
                        ):
                            self.page_produ += 1
                            print("------------------>   ", self.page_produ)
                        statut = self.menu2()
            ...
            # L'application se trouve au menu des substituts
                ...
                if entree_util == "s":
                    if (self.lignes_subst_prod_cat_bd % self.taille_page_subst) == 0:
                        if self.page_subst < (
                            self.lignes_subst_prod_cat_bd // self.taille_page_subst
                        ):
                            self.page_subst += 1
            ...
            # L'application se trouve au menu des favoris
                ...
                if entree_util == "s":
                    if (self.lignes_fav_bd % self.taille_page_favo) == 0:
                        if self.page_favo < (
                            self.lignes_fav_bd // self.taille_page_favo
                        ):
                            self.page_favo += 1
                        statut = self.menu6()
                    elif (self.lignes_fav_bd % self.taille_page_favo) > 0:
                        if self.page_favo < (
                            (self.lignes_fav_bd // self.taille_page_favo) + 1
                        ):
                            self.page_favo += 1
                        statut = self.menu6()
    ...
    def menu1(self):
        """ Cette méthode gère l'affichage du menu des catégories """
        ...
        for i in categories[0]:
            catego_display["selection"] = categories[0].index(i) + (
                (self.page_catego - 1) * self.taille_page_catego
            )
    ...
    def menu2(self):
        """ Cette méthode gère l'affichage du menu des produits """

        produits = self.gest_produits.get_produits_size_page(
            self.category_cursor["nom"],
            self.taille_page_produ,
            self.page_produ,
            self.curseur,
            self.connexion,
        )
    ...
    def menu3(self):
        """ Cette méthode gère l'affichage du menu des substituts """

        substituts = self.gest_produits.get_substitus_prod(
            self.category_cursor["nom"],
            self.product_cursor["nutriscore"],
            self.taille_page_subst,
            self.page_subst,
            self.curseur,
            self.connexion,
        )
        ...
        for i in substituts[0]:
            ...
            self.list_subst_display.append(
                {
                    "selection": substituts[0].index(i)
                    + ((self.page_produ - 1) * self.taille_page_produ),
                    "id": i[0],
                }
            )
    ...
    def menu4(self):
        """ Cette méthode gère l'affichage du menu de comparaison d'un produit à un substitut """
    ...
    def menu5(self):
        """ Cette méthode gère l'affichage du menu de la confirmation d'enregistrement d'un substitut """

        check_fav = self.gest_favoris.check_in_fav(
            self.product_cursor["id"],
            self.substitut_cursor["id"],
            self.curseur,
            self.connexion,
        )
        print("\n__________\nmenu5    |\n_________|\n")
    ...
    def menu6(self):
        """ Cette méthode gère l'affichage du menu des favoris """
        ...
        for i in favoris[0]:
            self.list_favo_display.append(
                {
                    "selection": favoris[0].index(i)
                    + (self.page_favo - 1) * self.taille_page_favo,
                    "id_prod": i[0],
                    "id_subs": i[1],
                }
            )
            ...
            produit_substitue = self.gest_favoris.get_name_prod(
                i[0], self.curseur, self.connexion
            )
            substitut = self.gest_favoris.get_name_prod(
                i[1], self.curseur, self.connexion
            )
            print(
    ...
    def menu7(self):
        """ Cette méthode gère l'affichage du menu des détails d'un favoris """

        infos_prods_substitue = self.gest_favoris.get_all_infos_prod(
            self.favori_cursor["id_prod"], self.curseur, self.connexion
        )
